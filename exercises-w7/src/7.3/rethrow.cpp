...
auto db_conn::try_connect(const std::string &uname, const std::string &pword) -> void {
    if (!active_) {
        ++n_attempts_;
        if (blacklist_.count(uname)) throw std::runtime_error("Could not establish connection: hsmith is not allowed to login.");
        if (n_attempts_ % 2 == 0) throw std::runtime_error("HeLp ;_; c0mpu73R c@ann0T c0mPut3 0w0");
        if (pword.size() >= 8) active_ = true;
    }
}
